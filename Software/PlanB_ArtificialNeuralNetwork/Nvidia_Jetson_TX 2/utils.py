...
Steuerbits_16[2]=1						# Neuronaler input							
...
def crop(image):				#TODO: Bildinhalt korrekt auswählen
    """
    Crop the image (removing the sky at the top and the car front at the bottom)
    """
    return image[100:-35, :, :] # remove the sky and the car front # [wieviel pixel von oben, wieviel pixel von unten, komplette breite]


def resize(image):			#TODO: Verstehen warum Eingang von Netz genau so?
    """
...
def preprocess(image):
    ...
    image = resize(image)
    image = rgb2yuv(image)
    #cv2.imshow('Aktueller Input in das Netz [RGB2YUV]',image)
    #key = cv2.waitKey(1) & 0xFF 
    return image


def preprocessKameraStream(image):
    """
    wie war das nochmal mit openCV wenn er einliest? ist das nicht BGR anstatt RGB?
    """
    image = crop(image)
    image = resize(image)
...
def augument(data_dir, center, steering_angle, range_x=100, range_y=10):
    ...
    #image, steering_angle = random_flip(image, steering_angle)		#weglassen, damit auto lernt rechts zu fahren
